document.addEventListener('DOMContentLoaded', function () {
    ...
    async function fetchEmployeeRows(firstDay, lastDate) {
      ...
      return employees.filter(emp => emp.role === 'regular_user' || emp.role === 'operator' || emp.role === 'noAccess_user').map(emp => `
        <tr data-user-id="${emp.id}">
          <td>
            <div class="employee">
              <div class="employee-info">${emp.name}<br><span> - Имя пользователя: ${emp.username}</span></div>
            </div>
          </td>
